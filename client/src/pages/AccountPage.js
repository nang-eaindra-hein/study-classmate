...
export default function AccountPage() {
  ...
  const confirmAvatarUpload = async () => {
    ...
    try {
      ...
      const res = await fetch('https://study-classmate-server.onrender.com/upload-avatar', {
        method: 'POST',
        body: form
      });
  ...
  const confirmUsernameChange = () => {
    if (diamonds < 30) {
      setMessageModal({ show: true, text: 'Not enough diamonds', type: 'error' });
      return;
    }
  ...
  const confirmPasswordChange = () => {
    if (!passwords.old || !passwords.new) {
      setMessageModal({ show: true, text: 'Fill both fields', type: 'error' });
      return;
    }
    fetch('https://study-classmate-server.onrender.com/change-password', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        username,
  ...
  const confirmBioChange = () => {
    ...
    fetch('https://study-classmate-server.onrender.com/update-bio', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      ...
      <div className="account-content">
        ...
        <div className="avatar-section">
          <img
            src={
              profile.avatarUrl
                ? `https://study-classmate-server.onrender.com${profile.avatarUrl}`
          ...
          {avatarPreview && (
            <div className="avatar-preview-container">
              ...
              <button className="btn" onClick={confirmAvatarUpload}>
                Upload
              </button>
            </div>
          )}
      ...
      {messageModal.show && (
        <div className="modal">
          <div className={`modal-content message-${messageModal.type}`}>
            ...
            <button onClick={() => setMessageModal({ show: false, text: '', type: 'info' })}>
              OK
            </button>
          </div>
        </div>
